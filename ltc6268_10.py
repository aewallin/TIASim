...
opa = tiasim.LTC6268_10()
gbwp = opa.GBWP


# read gain
d = numpy.genfromtxt('LTC6268-10/ltc6268-10_gain.txt', comments='#', delimiter=',')
f = [x[0]*1e6 for x in d]
aol = [pow(10,x[1]/20.0) for x in d]
print("Gain: ", f, aol)

# read phase
d = numpy.genfromtxt('LTC6268-10/ltc6268-10_phase.txt', comments='#', delimiter=',')
...
# read vnoise
d = numpy.genfromtxt('LTC6268-10/voltage_noise.txt', comments='#', delimiter=',')
fn = [x[0]*1e6 for x in d]
vn = [x[1]*1e-9 for x in d] # from nV/sqrt(Hz)
print("Vnoise: ", fn, vn)

# read inoise
d = numpy.genfromtxt('LTC6268-10/current_noise.txt', comments='#', delimiter=',')
fin = [x[0]*1e6 for x in d]
inoise = [x[1]*1e-12 for x in d] # from pA/sqrt(Hz)
...
plt.semilogx( fm, 20*numpy.log10( numpy.abs( opa.gain(fm) ) ),'-',label='TIASim LTC6268-10 model')
...
plt.ylabel('Gain / dB')
plt.title('LTC6268-10 Open-loop gain')
plt.grid()


#plt.figure()
plt.subplot(2,2,2)
plt.title('LTC6268-10 Open-loop phase')
plt.semilogx(fp, phase,'o',label='Datasheet')
plt.semilogx( fm,  360*numpy.angle( opa.gain(fm) )/(2*numpy.pi) ,'-',label='TIASim LTC6268-10 model')
plt.grid()
plt.xlabel('Frequency / Hz')
...
plt.subplot(2,2,3)
plt.title('LTC6268-10 voltage noise')
plt.loglog(fn, vn,'o',label='Datasheet')
plt.loglog( fm,  opa.voltage_noise(fm) ,'-',label='TIASim LTC6268-10 model')
plt.grid()
plt.xlabel('Frequency / Hz')
...
plt.subplot(2,2,4)
plt.title('LTC6268-10/current noise')
plt.loglog(fin, inoise,'o',label='Datasheet')
plt.loglog( fm,  opa.current_noise(fm) ,'-',label='TIASim LTC6268-10 model')
plt.grid()
plt.xlabel('Frequency / Hz')
